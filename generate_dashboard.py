...
def collect_data(keys, cipher):
    ...
    for i in range(14):
        ...
        trend.append({"d": d[5:], "orders": analysis["valid"], "revenue": analysis["revenue"]})
...
<div id="dash">
  <div class="page">
    ...
    <div class="sec">
      ...
      <div class="cc full"><div class="leg" style="margin-bottom:6px"><div class="li"><div class="ld" style="background:rgba(42,120,214,0.5)"></div>订单量</div><div class="li"><div class="ld" style="background:#C4536A"></div>销售额</div></div><canvas id="trendC" style="width:100%;height:220px"></canvas></div>
    </div>
...
function drawTrend(daily,selDay){
  ...
  const spots=[0,Math.round(n/3),Math.round(n*2/3),n-1];spots.forEach(i=>{if(i<n){ctx.fillText(daily[i].d,pad.l+cw*(i+.5)/n,h-4)}});
